...
export default function OrderTable(props) {
  const { item, customers, products } = props
  ...
  const rows = orderItems.map(itm => {
    const thisProduct = products.filter(prod => prod.id === itm.productId)[0]
    return {
      product: thisProduct.description, 
